...
<?php
	...
	$reponse = $bdd -> prepare("SELECT o.numOffer numOffer, o.picture picture_offer, o.price price_offer, o.priceType priceType_offer, o.qty quantity_offer, o.qtyType quantityType_offer, o.description description_offer, DATEDIFF(o.limitDate, NOW()) AS disponibility_offer, DATE_FORMAT(o.offerDate, '%d%/%m%/%Y à %Hh%i') AS offerDate, o.bio bio_offer, o.troc troc_offer, o.season saison_offer,  v.name variete_offer, v.category category_fruit, m.pseudo member_offer FROM offer o INNER JOIN variety v ON v.idVariety = o.Variety_idVariety INNER JOIN member m ON m.idMember = o.Member_idMember WHERE v.category= ?");
	$reponse -> execute(array(htmlspecialchars($_POST['category'])));

		while($donnees = $reponse -> fetch()) {

        if((isset($_SESSION['pseudo']) AND $donnees['member_offer'] != $_SESSION['pseudo']) OR !isset($_SESSION['pseudo'])) {
...
echo "<br/><div class='offre'>
      ...
      if(isset($_SESSION['pseudo']) AND $donnees['member_offer'] != $_SESSION['pseudo']) {
          ...
          echo "<p><form name='selectionOffre' action='selectionOffre_post.php?variable=$i' method='post' accept-charset='utf-8'>
                    <input type='submit' value='Sélectionner cette offre' class='bouton'/>
                </form></p></div></div><br/><br/>";
      }
